...
// Dashboard web dijalankan di laptop penerima lewat USB Serial. Akses lokal: http://127.0.0.1:8000, akses LAN: http://IP-LAPTOP-PENERIMA:8000
// =============================================================
...
struct SensorRecord {
  ...
  String node = "-";
  uint32_t counter = 0;
  String timestamp = "-";      // waktu dari TX/RX, tergantung konfigurasi
  ...
  // Data enkripsi Xoodyak yang akan dikirim ke Serial Monitor dan dashboard
...
String monotonicTimestampWIB(const String &candidateTimestamp) {
  // Mencegah tampilan waktu mundur saat paket invalid memakai clock RX,
  // sementara paket valid memakai rt dari TX. Karena tentu saja dua board
  // kecil harus membuat kita mengurus relativitas waktu versi hemat biaya.
...
void printRxEncryptionRow(const SensorRecord &rec) {
  Serial.print("ENC | NODE=");
  Serial.print(rec.node);

  Serial.print(" | CTR=");
  Serial.print((unsigned long)rec.counter);

  Serial.print(" | AD=");
  Serial.print(rec.associatedData);

  Serial.print(" | NONCE=");
  Serial.print(rec.nonceHex);

  Serial.print(" | CT=");
  Serial.print(rec.cipherHex);

  Serial.print(" | TAG=");
  Serial.println(rec.tagHex);
}
...
void processPacket(const String &packet, int rssi, float snr) {
  ...
  // Pakai waktu RX saat paket diterima.
  // Jika dashboard mengirim TIME_SYNC, waktu ini akan mengikuti jam laptop GMT+7.
  char nowWIB[24];
  getTimestampWIB(nowWIB, sizeof(nowWIB));
  rec.timestamp = String(nowWIB);

  rec.timestamp = monotonicTimestampWIB(rec.timestamp);
  ...
  // Simpan hasil enkripsi yang diterima dari paket LoRa
  rec.associatedData = associatedData;
  rec.nonceHex = nonceHex;
  rec.cipherHex = cipherHex;
  rec.tagHex = tagHex;
...
void setup() {
  ...
  Serial.println("Timestamp valid diambil dari TX field rt=... setelah decrypt, lalu dibuat monotonic agar tidak mundur.");
  Serial.println("TIME_SYNC dari dashboard bersifat opsional. Timestamp utama dashboard tetap memakai jam laptop GMT+7.");
